...
THINGS are either registrations or unregisterations."
  (cl-loop
   ...
   collect (eglot--dbind Registration (&key id method registerOptions) thing
...
(defun eglot--apply-workspace-edit (wedit &optional confirm)
  ...
  (eglot--dbind WorkspaceEdit (&key changes documentChanges) wedit
    (let ((prepared
           (mapcar (eglot--lambda TextDocumentEdit (&key textDocument edits)
...
(defun eglot-code-actions (&optional beg end)
  ...
  (let* ((server (eglot--current-server-or-lose))
         ...
         (menu-items
          (or (mapcar (eglot--lambda CodeAction
                          (&key title edit command arguments)
...
(cl-defun eglot--register-workspace/didChangeWatchedFiles (server &key id watchers)
  ...
  (let* (success
         (globs (mapcar (eglot--lambda FileSystemWatcher (&key globPattern)
                          globPattern)
                        watchers)))
    (cl-labels
        ((handle-event
